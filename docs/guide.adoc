...
The workspace list in the tray menu shows one label for each unique formatted window value.
By default, labels contain application names. You can include other window information, such as the window title:
...
The available interpolation variables are the same as for xref:commands.adoc#list-windows[`list-windows --format`].
